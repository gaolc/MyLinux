...
- name: check frontend process port 
  include_tasks: check_frontend_process_port.yml
  when: inventory_hostname in groups['frontendServer']
- name: check backend process port
  include_tasks: check_backend_process_port.yml
  when: (inventoy_hostname in groups['fmsServer'] or inventory_hostname in groups['jobServer'])
...
- wait_for:
      port: "{{ item.port}}"
      delay: "{{fxmss_server.delay}}"
      timeout: "{{fxmss_server.timeout}}"
  with_items:
      - { port: '8080'}
      - { port: '18083'}
#vars/main.yml
